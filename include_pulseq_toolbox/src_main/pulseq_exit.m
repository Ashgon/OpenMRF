...
if isfield(FOV,'fov_x') && isfield(FOV,'fov_y') && isfield(FOV,'dz')
    seq.setDefinition('FOV', [FOV.fov_x FOV.fov_y FOV.dz]);
elseif isfield(FOV,'fov_xy') && isfield(FOV,'dz')
    seq.setDefinition('FOV', [FOV.fov_xy FOV.fov_xy FOV.dz]);
end
...
if flag_sound==1
   [gradSpectrum.gs, gradSpectrum.gs_tot, gradSpectrum.faxis] = pulseq_get_gradSpectrum(seq, system);
   seq.sound();
else
    gradSpectrum = [];
end
...
if flag_backup>0
    ...
    if check_duplicate
        ...
        PULSEQ.gradSpectrum   = gradSpectrum;
...
MRF_sim_on_the_fly(seq, external_path, flag_mrf); % perform EPG simulation
