function M_dict = MRF_sim_brute_force(SIM, P, z, dw0_distr, comp_energy)

% ----- versions: -----
% V1: 17.04.2025; M. Gram; University of Wuerzburg
% V2: 15.06.2025: M. Gram; University of Wuerzburg; adiabatic spin-locking
% V3: 02.08.2025: M. Gram; University of Wuerzburg; load/save compressed dictionary
...
% rf pulses are simulated step-by-step on a 1us or 10us raster
...
tic;
fprintf('\n');
fprintf(['calculating dictionary: ' num2str(N_dict) ' atoms via brute force Bloch \n']);
...
parfor j = 1:N_dict
    ...
    switch(ID(k))
        ...
        case {2, 3} % global rf excitation
            M = sim_relax_deph_rf(M, DT(k), RF_(k), T1(j), T2(j), DT(k)*dw0_);
    
        case 4 % spin-lock
            M = sim_spin_lock(M, DT(k), RF_(k), T1p(j), T2p(j), m1p(j), dw0_);
    
        case {5, 6, 7} % slice selective rf excitation
            M = sim_relax_deph_rf(M, DT(k), RF_(k), T1(j), T2(j), DT(k)*(GZ(k)*z+dw0_));
        ...
        case 12 % adiabatic spin-lock
            M = sim_adiabatic_sl(M, DT(k), RF_(k), T1p_adia(j), DT(k)*dw0_);
...
%% ---------- simulate: relaxation, depashing & rf -----------
function M = sim_relax_deph_rf(M, dt, rf, T1, T2, psi)
    E1        = exp(-dt/T1);
    E2        = exp(-dt/T2);
    cos_psi   = cos(psi);
    sin_psi   = sin(psi);
    cos_phi   = cos(angle(rf));
    sin_phi   = sin(angle(rf));
    cos_alpha = cos(abs(rf) * dt);
    sin_alpha = sin(abs(rf) * dt);
    RF00      = cos_phi^2 + sin_phi^2 * cos_alpha;
    RF01      = cos_phi * sin_phi * (1- cos_alpha);
    RF02      = -sin_phi * sin_alpha;
    RF10      = cos_phi * sin_phi * (1- cos_alpha);
    RF11      = sin_phi^2 + cos_phi^2 * cos_alpha;
    RF12      = cos_phi * sin_alpha;
    RF20      = sin_phi * sin_alpha;
    RF21      = -cos_phi * sin_alpha;
    RF22      = cos_alpha;
    O         = zeros(size(psi));
    I         = ones(size(psi));    
    B         = [ E2*RF00*cos_psi+E2*RF10*sin_psi, E2*RF01*cos_psi+E2*RF11*sin_psi, E2*RF02*cos_psi+E2*RF12*sin_psi, O;
                  E2*RF10*cos_psi-E2*RF00*sin_psi, E2*RF11*cos_psi-E2*RF01*sin_psi, E2*RF12*cos_psi-E2*RF02*sin_psi, O;
                  E1*RF20*I,                       E1*RF21*I,                       E1*RF22*I,                       I-E1;
                  O,                               O,                               O,                               I ];
    M = matrix_vector_iso_prod(M, B);
end
...
%% ---------- simulate: adiabatic spin-locking -----------
function M = sim_adiabatic_sl(M, dt, rf, T1p_adia, psi)
    E1        = exp(-dt/T1p_adia);
    cos_psi   = cos(psi);
    sin_psi   = sin(psi);
    cos_phi   = cos(angle(rf));
    ...
    O         = zeros(size(psi));
    I         = ones(size(psi));    
    B         = [ E1*RF00*cos_psi+E1*RF10*sin_psi, E1*RF01*cos_psi+E1*RF11*sin_psi, E1*RF02*cos_psi+E1*RF12*sin_psi, O;
                  E1*RF10*cos_psi-E1*RF00*sin_psi, E1*RF11*cos_psi-E1*RF01*sin_psi, E1*RF12*cos_psi-E1*RF02*sin_psi, O;
                  E1*RF20*I,                       E1*RF21*I,                       E1*RF22*I,                       O;
                  O,                               O,                               O,                               I ];
    M = matrix_vector_iso_prod(M, B);
end
